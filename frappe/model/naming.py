   ...
   *Example:*
    ...
    if key == "hash":
        ...
        for i in range(0, 100):         # limit recursion to prevent endless loop
            ...
            if not frappe.db.exists(doctype, hashname):
                return hashname
        frappe.throw( _("Unable to find an available hash after 100 iterations on {0}. Please increase the hash length in the system settings").format(doctype) )
